...
-   The last city in `txhousing` is **`r tail(txhousing, n = 1)$city`**. 
:::
...
-   The `listings` variable in this data set represents the number of total active listings. 
:::
...
dallas_summary <- dallas_sub %>% 
  group_by(month) %>% 
  summarize(mean_prct_sold = mean(sales))
head(dallas_summary)
...
-   This code chunk outputs data as one piece of a data set. The data represents Dallas listings sold in the years 2012 and 2013 on average per month in `desc`ending order. This is different from the outputs from questions #4-#7 because my outputs demonstrate the `listings` in Dallas in 2012 and 2013, but the numbers for `mean percentage sold` range from 3900 to 5800, whereas these numbers range from 30 to 39. Apart from this, there seems to be no difference between the two codes that were inputted. 
:::
...
-   **`r tail(listings2, n = 1)` ** had the lowest number of `listings` in January of 2015, which can be found by inputting the code above or typing in to the console `View(txhousing)`
:::
...
-   In August of 2012, there were 7112 houses sold in Texas. 
:::
...
```{r}
totalSales1 <- txhousing %>% 
  filter(year == 2000 | year == 2001, city == "Austin" | city == "Dallas") %>%
  group_by(city) %>%
